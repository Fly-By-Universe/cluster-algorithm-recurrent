...
def construction(nodeList: list[Node], nns: dict[int], iteration: int):
    ...
    Args:
        ...
        nns (dict[int]): 最近邻字典，键为节点 ID，值为最近邻节点 ID
        iteration (int): 当前迭代次数
    
    Returns:
        dict: 根节点字典，键为节点 ID，值为 Node 对象
    """
    roots = {}
    # 创建 ID 到 Node 的映射字典
    nodeDict = {node.id: node for node in nodeList}
    candidates = np.array([node.id for node in nodeList])  # 使用节点 ID 初始化候选列表
    while len(candidates) > 0:
        link = np.array([])  # 记录当前路径中的节点
        ...
        while True:
            ...
            nodeDict[n].set_iteration(iteration)  # 使用 nodeDict[n] 而非 nodeList[n]
            j = nns[n]  # 获取最近邻节点 ID
            if j in link and j not in nodeDict[n].cannot_link_node:
                root_node = nodeDict[n]
                ...
                for node_id in link:
                    ...
                    root_node.must_link_node = list(set(root_node.must_link_node + nodeDict[node_id].must_link_node))
                    root_node.cannot_link_node = list(set(root_node.cannot_link_node + nodeDict[node_id].cannot_link_node))
                break
            elif j not in candidates and j not in nodeDict[n].cannot_link_node:
                nodeDict[n].add_adjacent_node(nodeDict[j])
                nodeDict[j].add_adjacent_node(nodeDict[n])
                candidates = np.setdiff1d(candidates, link)
                break
            else:
                if j not in nodeDict[n].cannot_link_node:
                    nodeDict[n].add_adjacent_node(nodeDict[j])
                    nodeDict[j].add_adjacent_node(nodeDict[n])
                    n = j
                else:
...
def query_top(root_keys, nodes_dict, query_times):
    ...
    for i in range(len(root_keys)):
        for j in range(i + 1, len(root_keys)):
            ...
            # 检查是否已通过传递性关联
            if uf.find(node_i.id) == uf.find(node_j.id):
                continue  # 跳过查询
            ...
            if result == "must_link":
                uf.union(node_i.id, node_j.id)  # 并查集合并
                
                # 始终存储节点ID
                if node_j.id not in node_i.must_link_node:
                    node_i.must_link_node.append(node_j.id)
                if node_i.id not in node_j.must_link_node:
                    node_j.must_link_node.append(node_i.id)

                # 合并 must_link 约束 (使用节点ID)
                node_i_must_link = set(node_i.must_link_node)
                node_j_must_link = set(node_j.must_link_node)
                
                # 更新两个节点的 must_link 列表
                node_i.must_link_node = list(node_i_must_link.union(node_j_must_link))
                node_j.must_link_node = list(node_i_must_link.union(node_j_must_link))

            elif result == "cannot_link":
                # 存储 cannot_link 关系 (使用节点ID)
                if node_j.id not in node_i.cannot_link_node:
                    node_i.cannot_link_node.append(node_j.id)
                if node_i.id not in node_j.cannot_link_node:
                    node_j.cannot_link_node.append(node_i.id)

                # 传递 cannot_link 约束
                # node_i 不能连接到 node_j 的所有 must-link
                for must_link_id in node_j.must_link_node:
                    if must_link_id not in node_i.cannot_link_node:
                        node_i.cannot_link_node.append(must_link_id)
                
                # node_j 不能连接到 node_i 的所有 must-link
                for must_link_id in node_i.must_link_node:
                    if must_link_id not in node_j.cannot_link_node:
                        node_j.cannot_link_node.append(must_link_id)
...
def run(task: Task, data_override=None):
    ...
    while len(nodeList) > K and len(nodeList) > 3:
        ...
        roots = construction(nodeList=list(nodeList.values()), nns=nns, iteration=iteration)
...
def generate_synthetic_data(n_samples=3000, n_features=2, n_clusters=3, cluster_std=2.8, random_state=100):
    """
    生成二维平面数据点，并增加随机性。
