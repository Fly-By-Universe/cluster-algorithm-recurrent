...
def spread(node):
    if node.root_judge[node.iteration] == 1:  # 如果 node 原先是根节点，直接返回
        return  
    elif node.root_judge[node.iteration] == 0 and node.iteration > 0:  # 如果 node 原来不是根节点，则需要删除其邻接节点
        to_delete = []  
        for i in list(node.adjacentNode.values()):  # 转换为列表，避免修改字典时报错
            if node.iteration == i.iteration and i.id not in node.must_link_node:
    ...
    elif node.root_judge[node.iteration] == 0 and node.iteration == 0: #如果node为叶子节点，且不是密度峰值点，则直接删除其邻接节点
        to_delete = [adj_node for adj_node in node.adjacentNode.values() if adj_node.id not in node.must_link_node] # 先记录需要删除的节点
        for i in to_delete:
            del i.adjacentNode[node.id]
            if i.id in node.adjacentNode:  # 如果 i 在 node 的邻接节点中，则删除
                del node.adjacentNode[i.id]
...
def run(task: Task, data_override=None):
    ...
    while len(nodeList) >= 3 and current_cluster_count_1 != current_cluster_count_2:
...
def plot_all_datasets_results():
    plt.rcParams['font.sans-serif'] = ['SimHei']
    
    # 创建保存图表的目录
    os.makedirs('C:/Users/DELL/OneDrive/桌面/dezh123/123', exist_ok=True)
    
    # 计算行数和列数
    n_datasets = len(all_datasets_results)
    if n_datasets <= 3:
        n_cols = n_datasets
        n_rows = 1
    else:
        n_cols = 3
        n_rows = (n_datasets + 2) // 3
    ...
    # 遍历所有数据集结果
    for i, (dataset_name, result) in enumerate(all_datasets_results.items()):
        row = i // n_cols
        col = i % n_cols
        
        query_counts = np.array(result['query_counts'])
        ari_values = np.array(result['ari_values'])
        
        if query_counts[0] != 0 or ari_values[0] != 0:
            query_counts = np.insert(query_counts, 0, 0)
            ari_values = np.insert(ari_values, 0, 0)
        
        ax = axes[row, col]
        ax.plot(query_counts, ari_values, linestyle='-', linewidth=1, color='blue')
        ax.set_title(f'{dataset_name}', pad=20)  
        ax.set_xlabel('查询次数', labelpad=10)  
        ax.set_ylabel('ARI 值', labelpad=10)    
        ax.set_ylim(0, 1.0)  
        ax.set_xlim(0, max(query_counts)*1.02)  
        ax.grid(True, linestyle='--', alpha=0.7)  
        
        # 在(0,0)点添加一个明显的点
        ax.scatter([0], [0], color='red', s=30, zorder=5)
    ...
    # 调整子图之间的间距
    plt.tight_layout(h_pad=2.5, w_pad=1.5)  # 增加子图之间的垂直和水平间距
    plt.show()
    plt.close()
