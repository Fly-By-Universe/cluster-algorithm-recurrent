...
def connect_roots(rebuild_roots, roots, snns, nodeList: list[Node],iteration:int):
    candidates = np.array(rebuild_roots).reshape(-1)
    for root in rebuild_roots:
        root_node_0 = nodeList[root[0]]
        root_node_1 = nodeList[root[1]]
        
        # 新增条件判断：检查 cannot_link 关系，避免不允许的连接
        if (root_node_1 in root_node_0.cannot_link_node) or (root_node_0 in root_node_1.cannot_link_node):
            continue  # 跳过连接
        else:
            roots.pop(root[0])
            ...
            if left_connect_node <= right_connect_node:
                big_node = nodeList[snns[root[1]]]
                small_node = nodeList[root[1]]
            else:
                big_node = nodeList[snns[root[0]]]
                small_node = nodeList[root[0]]
            ...
            # 如果小节点在 candidates 中，则需要更新 roots
...
def rebuild(snns: dict[int], roots: list[Node], nodeList: list[Node], iteration: int):
    ...
    connect_roots(rebuild_roots, roots, snns, nodeList,iteration)

def construction(nodeList: list[Node], nns: dict[int], iteration: int):
    roots = {}
    # 使用列表记录剩余候选节点（原来使用 np.array，这里改为 Python list 便于成员检测）
    candidates = list(nodeList.keys())
    
    while len(candidates) > 0:
        link = []  # 使用 Python list 记录路径上的节点 id
        ...
        while True:
            ...
            # 如果 j 已在当前路径中，且不在当前节点的 cannot_link 列表中，则认为形成了循环，确定当前聚类根
            if j in link and j not in nodeList[n].cannot_link_node:
                root_node = nodeList[n]
                roots[root_node.id] = root_node
                # 从候选列表中移除已访问的节点
                candidates = [c for c in candidates if c not in link]
                
                # 将路径上所有节点的约束合并到 root_node 中
                for node_id in link:
                    node_id = int(node_id)  # 确保为整数
                    if iteration > 1:
                        root_node.must_link_node.append(node_id)
                    root_node.must_link_node = list(set(
                        root_node.must_link_node +
                        [x if isinstance(x, int) else x.id for x in nodeList[node_id].must_link_node]
                    ))
                    root_node.cannot_link_node = list(set(
                        root_node.cannot_link_node +
                        [x if isinstance(x, int) else x.id for x in nodeList[node_id].cannot_link_node]
                    ))
                break

            # 若 j 不在候选列表中且不在当前节点的 cannot_link 列表中，则建立边后结束当前路径
            elif j not in candidates and j not in nodeList[n].cannot_link_node:
                nodeList[n].add_adjacent_node(nodeList[j])
                nodeList[j].add_adjacent_node(nodeList[n])
                candidates = [c for c in candidates if c not in link]
                break

            # 否则继续延伸路径：建立相邻边，然后将 j 作为下一个节点
            nodeList[n].add_adjacent_node(nodeList[j])
            nodeList[j].add_adjacent_node(nodeList[n])
            n = j
...
class UnionFind:
    ...
    def find(self, node_id):
        ...
        while self.parent[node_id] != node_id:
            self.parent[node_id] = self.parent[self.parent[node_id]]  
...
def query_top(root_keys, nodes_dict, query_times):
    for i in range(len(root_keys)):
        for j in range(i + 1, len(root_keys)):
            node_i = nodes_dict[root_keys[i]]
            node_j = nodes_dict[root_keys[j]]
            ...
            if result == "must_link":
                node_i.must_link_node.append(node_j)
                node_i.must_link_node = list(set(node_i.must_link_node + node_j.must_link_node))
                node_j.must_link_node.append(node_i)
                node_j.must_link_node = list(set(node_j.must_link_node + node_i.must_link_node))
            elif result == "cannot_link":
                node_i.cannot_link_node.append(node_j)
                node_i.cannot_link_node = list(set(node_i.must_link_node + node_j.must_link_node))
                node_j.cannot_link_node.append(node_i)
                node_j.cannot_link_node = list(set(node_j.must_link_node + node_i.must_link_node))
    return query_times
        ...
        for must_link_id in node.must_link_node:
            ...
            if must_link_id in nodeList:
                ...
                plt.plot([node.data[0], must_link_node.data[0]],
                         ...
                         'r-', linewidth=1.5)  # Must-Link (红色实线)
...
def run(task: Task, data_override=None):
    ...
    while len(nodeList) > K and len(nodeList) > 3:
...
def generate_synthetic_data(n_samples=300, n_features=2, n_clusters=3, cluster_std=2, random_state=100):
    """
    生成二维平面数据点，并增加随机性。
    """
...
if __name__ == '__main__':
    data, label, K = generate_synthetic_data(n_samples=3000, n_clusters=5)
