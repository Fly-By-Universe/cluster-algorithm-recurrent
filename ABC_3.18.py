...
class Node(object):
    ...
    def __init__(self, node_id, data, label, labelName):
        ...
        self.node_num = 0
        self.parent = None #双亲节点，如果没找到就是-1
        # 结果
...
def run(task: Task, data_override=None):
    ...
    nodeList_copy = {i: Node(i, data[i], label[i], label[i]) for i in range(len(data))}
    iteration = 0
    start = time.time()
    ...
    cluster_centers = extract_cluster_centers(current_roots, nodeList_copy)
    ...
    top_nodes = get_top_k_uncertain_nodes(all_nodes, top_k = len(nodeList_copy))     
    ...
    for node in top_nodes:
        ...
        node_copy = node
        while node_copy.parent != node_copy:
        ...
        if ans == "must_link":
            ...
            if query_times_2 == len(current_roots):
                node.label = len(current_roots)
                current_roots.append(node.id)
                spread(node, nodeList_copy, current_roots)   #3.17日写到这里，该增加对于目前节点的标签向子节点传播的函数！！
        print(node.id)
        label_true, label_pred = get_distribute(current_roots, nodeList_copy, len(label))
