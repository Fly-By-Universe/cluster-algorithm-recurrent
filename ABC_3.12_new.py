...
def connect_roots(rebuild_roots, roots, snns, nodeList: list[Node],iteration:int):
    ...
    for root in rebuild_roots:
        ...
        if (root_node_1 in root_node_0.cannot_link_node) or (root_node_0 in root_node_1.cannot_link_node):
            ...
            if iteration > 0:
                big_node.must_link_node.append(small_node)
                small_node.must_link_node.append(big_node)
    ...
    while len(candidates) > 0:
        ...
        while True:
            ...
            if j in link and j not in nodeList[n].cannot_link_node:
                ...
                # **合并所有子节点的 must_link_node 和 cannot_link_node 到 root**
                for node_id in link:
                    if iteration > 1:
                        root_node.must_link_node.append(node_id)
                    root_node.must_link_node = list(set(root_node.must_link_node + nodeList[node_id].must_link_node))
                    root_node.cannot_link_node = list(set(root_node.cannot_link_node + nodeList[node_id].cannot_link_node))
...
def query_top(root_keys, nodes_dict, query_times):
    ...
    for i in range(len(root_keys)):
        for j in range(i + 1, len(root_keys)):
            ...
            if result == "must_link":
                uf.union(node_i.id, node_j.id)  # 更新并查集
